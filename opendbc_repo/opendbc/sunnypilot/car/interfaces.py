...
from opendbc.sunnypilot.car.chrysler.values_ext import ChryslerFlagsSP, JEEPS
...
from opendbc.sunnypilot.car.volkswagen.values import VolkswagenHCAMode, VOLKSWAGEN_HCA_MODE_MAP, VOLKSWAGEN_HCA_DELTA_RATE_MAP
...
def _initialize_jeep_brake_hold(CP: structs.CarParams, CP_SP: structs.CarParamsSP, params_dict: dict[str, str]) -> None:
  # Gated strictly to supported Jeep platforms. JEEP_CHEROKEE_5TH_GEN (CUSW)
  # uses a different CAN stack and isn't covered by this implementation.
  if CP.brand == 'chrysler' and CP.carFingerprint in JEEPS:
    try:
      jeep_brake_hold = int(params_dict.get("JeepBrakeHold", 0))
    except (ValueError, TypeError):
      jeep_brake_hold = 0

    if jeep_brake_hold > 0:
      CP_SP.flags |= ChryslerFlagsSP.BRAKE_HOLD.value
      if jeep_brake_hold == 2:
        CP_SP.flags |= ChryslerFlagsSP.BRAKE_HOLD_OFFSET_3.value
